...
export default function Maquinas() {
  ...
  const openEdit = (m) => {
    setForm({ nome: m.nome, custo_hora: m.custo_hora });
    setEditId(m.id);
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    const body = { nome: form.nome, custo_hora: Number(form.custo_hora) || 0 };
  ...
  return (
    <div>
      <PageHeader
        title="Máquinas"
        subtitle="Equipamentos e custo/hora usados no cálculo do roteiro"
        actions={
      ...
      <div className="bg-white border border-gray-200 rounded-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody data-testid="maquinas-table">
            ...
            {items.length === 0 && (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-400 text-sm">Sem máquinas. Crie a primeira.</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              ...
              <input data-testid="maquina-custo-input" type="number" step="0.01" value={form.custo_hora} onChange={(e) => setForm({ ...form, custo_hora: e.target.value })} className="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm tabular-nums focus:outline-none focus:ring-2 focus:ring-black/20 focus:border-black" />
            </div>
          </div>
          <DialogFooter>
